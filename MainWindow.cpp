...
void MainWindow::Hideinit()
{

ui->editorTab->clear();
ui->codeEditor->hide();
}
void MainWindow::textChange(int num )
{
      QString ntab=ui->editorTab->tabText(num);
      QString Editortext=ui->codeEditor->toPlainText();
    if(!m_map.contains(nowpage))
    {
            m_map.insert(nowpage, Editortext);
    }
    else
    {
            m_map[nowpage] = Editortext;
    }
    ui->codeEditor->clear();
     if(m_map.contains(ntab))
     {
         ui->codeEditor->setText(m_map[ntab]);
     }
     nowpage=ntab;
}
void MainWindow::createPage(QString name)//创建新页面
{
    pagenum++;
    auto empty = new QWidget();
    ui->codeEditor->show();
    ui->editorTab->insertTab(pagenum,empty,name);
    connect(ui->editorTab,SIGNAL(tabBarClicked(int)),this,SLOT(textChange(int)));
    connect(ui->editorTab,SIGNAL(tabCloseRequested(int )),this,SLOT(cleartext(int)));
    if(pagenum==1)
    {
        nowpage=name;
    }
...
void MainWindow::on_actionNew_triggered()
{
    ...
    QString str=QString::number(pagenum,16);;

    QString pagename="空白页"+str;
   createPage(pagename);
...
void MainWindow::on_actionOpen_triggered()
{
        ...
        QString fileName = QFileDialog::getOpenFileName(
                    this, tr("open  file"),
                    "./", tr("选择文件(*.c *.c++ *.h *.cpp *.txt *.hpp);;All files (*.*)"));

        if(fileName.isEmpty())
        {
            QMessageBox mesg;
            mesg.warning(this,"警告","打开失败!");
            return;
        }
        else
        {
            ...
            pagenum++;
            auto empty = new QWidget();
            ui->editorTab->insertTab(pagenum,empty,tabName);
            QFile file(fileName);
            ...
            else
            {
              ...
              m_map[tabName]=(text);
              ui->codeEditor->clear();
              ui->codeEditor->setText(m_map[tabName]) ;
            }
            if(!save_map.contains(tabName))
            {
                    save_map.insert(tabName, fileName);
            }
            else
            {

                    save_map[tabName] = fileName;
            }
            connect(ui->editorTab,SIGNAL(tabBarClicked(int)),this,SLOT(textChange(int)));
            connect(ui->editorTab,SIGNAL(tabCloseRequested(int )),this,SLOT(cleartext(int)));
...
void MainWindow::cleartext(int empty)
{
   ui->codeEditor->clear();
...
void MainWindow::Save_FileAs()
{
    QFileDialog fileDialog;
    QString sfileName = fileDialog.getSaveFileName(this,tr("Open File"),"/home",tr("Text File(*.txt)"));
    if(sfileName == "")
    {
        return;
    }
    ...
    else
    {
        ...
        QString str = ui->codeEditor->toPlainText();
        textStream<<str;
        QMessageBox::warning(this,tr("提示"),tr("保存文件成功"));
        if(!save_map.contains(nowpage))
        {
                save_map.insert(nowpage, sfileName);
        }
        else
        {
                save_map[nowpage] = sfileName;
        }
        file.close();
    }
}
